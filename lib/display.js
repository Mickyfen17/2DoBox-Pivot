...
const creatCard = newTodoContent => {
  ...
  return $(`
    <article id="${id}" class="todo-card ${isComplete}">
...
export const displayTodo = newTodoContent => {
  ...
  let titleNode = $(`<h5 class='todo-title edit' contenteditable>${title}</h5>`);
  let bodyNode = $(`<p class='todo-body edit' contenteditable>${body}</p>`);
